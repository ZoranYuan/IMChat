      ...
      <footer class="grid flex-none gap-2 border-t border-[#eef2f8] bg-white px-8 pb-3 pt-2.5 max-[767px]:px-3">
        ...
        <div class="mx-auto w-full max-w-[780px] min-[1500px]:max-w-[840px]">
          <div :class="isResizingComposer ? 'select-none' : ''" :style="{ height: `${composerHeight}px` }" class="relative flex w-full min-h-0 transition-none">
            ...
            <div class="flex min-h-0 min-w-0 flex-1 flex-col px-1 pt-3">
              <div class="flex h-8 flex-none items-center gap-1 text-[#7d7890]">
              ...
              <textarea v-model="draft" rows="1" placeholder="输入消息..." class="min-h-0 min-w-0 flex-1 resize-none overflow-y-auto border-0 bg-transparent px-2 py-2 text-[12px] leading-5 text-[#475467] outline-0 placeholder:text-[#a1acba]" @keydown.enter="handleEnter"></textarea>
              <div class="flex h-11 flex-none items-center justify-end">
                <button class="flex h-10 w-[88px] items-center justify-center gap-2 rounded-[9px] bg-gradient-to-br from-[#7355ff] to-[#4b2bd7] text-[13px] font-bold text-white shadow-[0_10px_20px_rgba(91,53,245,0.2)] transition-[box-shadow,transform] duration-150 hover:-translate-y-0.5 hover:shadow-[0_14px_26px_rgba(91,53,245,0.28)] disabled:cursor-not-allowed disabled:opacity-50" type="button" title="发送消息" :disabled="!draft.trim() || connection !== 'connected' || sending" @click="submit"><span>发送</span><Send :size="16" /></button>
              </div>
            </div>
          </div>
        </div>

...
func (c *RoomCache) GetMember(
	...
	if err := json.Unmarshal(data, &entry); err != nil {
		...
		return nil, false, fmt.Errorf("decode room member cache: %w", err)
	}
	if !entry.Found {
		return nil, true, nil
	}
...
func (c *RoomCache) SetMember(
	ctx context.Context,
	roomID, userID string,
	state *roomcache.MemberState,
) error {
	if state == nil {
		return fmt.Errorf("room member state is nil")
	}
...
func (c *RoomCache) SetMemberNotFound(ctx context.Context, roomID, userID string) error {
	return c.setMemberEntry(ctx, roomID, userID, roomMemberEntry{}, roomMemberNegativeTTL)
}

func (c *RoomCache) DeleteMember(ctx context.Context, roomID, userID string) error {
	return c.store.Del(ctx, RoomMemberKey(roomID, userID))
}

func (c *RoomCache) setMemberEntry(
	ctx context.Context,
	roomID, userID string,
...
func (c *RoomCache) SetMemberIDs(ctx context.Context, roomID string, userIDs []string) error {
...
func (c *RoomCache) GetMemberIDs(ctx context.Context, roomID string) ([]string, bool, error) {
	const script = `
	...
	data, ok := result.([]any)
	if !ok || len(data) != 2 {
		return nil, false, fmt.Errorf("invalid room member list cache result")
	}
	cached, ok := data[0].(int64)
	if !ok {
		return nil, false, fmt.Errorf("invalid room member list cache marker")
	}
	rawMembers, ok := data[1].([]any)
	if !ok {
		return nil, false, fmt.Errorf("invalid room member list cache values")
	}
	...
	for _, raw := range rawMembers {
		...
		case []byte:
			member = string(value)
		default:
			return nil, false, fmt.Errorf("invalid room member ID type %T", raw)
		}
...
func (c *RoomCache) DeleteMemberIDs(ctx context.Context, roomID string) error {

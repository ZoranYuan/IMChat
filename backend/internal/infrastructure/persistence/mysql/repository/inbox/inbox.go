...
func (r *InboxRepository) TryClaim(ctx context.Context, eventID, eventType string, now, staleBefore time.Time) (bool, string, error) {
	if eventID == "" {
		return true, "", nil
	}
	...
	if !claimed {
		return false, "", inboxport.ErrLeaseLost
	}
	return claimed, lockToken, nil
}
